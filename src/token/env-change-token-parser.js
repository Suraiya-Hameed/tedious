...
/*
module.exports = function (parser, colMetadata, options, callback) {
  temp(parser, colMetadata, options, callback).then(() => {
    console.log('Done Resoling promise');
    return;
  });
};
*/
async function temp(parser, colMetadata, options, callback) {
  ...
  let { newValue, oldValue } = await _readNewAndOldValue(parser, length, type);
...
async function _readNewAndOldValue(parser, length, type, callback) {
  let newValue,oldValue;
  switch (type.name) {
    case 'DATABASE':
    case 'LANGUAGE':
